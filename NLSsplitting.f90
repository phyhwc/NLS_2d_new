    ...
    ! i*u_t+delta*|u|^2u+u_{xx}+u_{yy}=0 changed into i*u_t+delta*|u|^2u+beta*u_{xx}+gamma*u_{yy}=0
    ...
    IMPLICIT NONE 
    ! Declare variables
    INTEGER(kind=4), PARAMETER ::  Nx=2**10
    INTEGER(kind=4), PARAMETER ::  Ny=2**10
    INTEGER(kind=4), PARAMETER ::  Nt=200000
    INTEGER(kind=4), PARAMETER ::  plotgap=500
    REAL(kind=8), PARAMETER    ::  &
    pi=3.14159265358979323846264338327950288419716939937510d0
    REAL(kind=8), PARAMETER    ::  Lx=512.0d0
    REAL(kind=8), PARAMETER    ::  Ly=512.0d0

    REAL(kind=8), PARAMETER    ::  delta=-1.0d0
    REAL(kind=8), PARAMETER    ::  beta=1.0d0
    REAL(kind=8), PARAMETER    ::  gamma=3.0d0
    REAL(kind=8), PARAMETER    ::  p=2.0d0
    REAL(kind=8), PARAMETER    ::  q=3.0d0
    REAL(kind=8), PARAMETER    ::  kk=2.0d0
    REAL(kind=8), PARAMETER    ::  ll=3.0d0
    REAL(kind=8), PARAMETER    ::  epsilon=1.0d0
    REAL(kind=8), PARAMETER    ::  xi0=1.0d0
    REAL(kind=8), PARAMETER    ::  eta0=1.0d0   
    REAL(kind=8), PARAMETER    ::  t=0.0d0


    REAL(kind=8)               ::  dt=2000.0d0/Nt
    ...
    CHARACTER*100              ::  name_config,number_file,cmdchar

    !-----------------------创建文件夹data--------！
                        
                    cmdchar='mkdir data'
                    call system(adjustl(trim(cmdchar)))
                       
    !________________________创建文件夹data--------！
...
!    unax(1:Nx,j) = sqrt(-(dble(2 * beta * p ** 2) + 0.2D1 * 0.5772156649D&
!     0 * q ** 2) / delta) * cexp((0, -1) * (dble(kk * x(1:Nx)) + ll * y - &
!     (dble(beta * kk ** 2) + 0.5772156649D0 * ll ** 2 + dble(beta * p ** 2) / &
!     0.2D1 + 0.5772156649D0 * q ** 2 / 0.2D1) * t + xi0)) * (0.1D1 - epsilon&
!     * exp(-(dble(2 * beta * kk * p) + 0.2D1 * 0.5772156649D0 * ll * q)&
!      * t + dble(p * x(1:Nx)) + q * y + eta0)) / (0.1D1 + epsilon * exp(-(dble(2 &
!     * beta * kk * p) + 0.2D1 * 0.5772156649D0 * ll * q) * t + dble(p * x(1:Nx)&
!     ) + q * y + eta0)) / 2


     unax(1:Nx,j) = sqrt(-(dble(2 * beta * p ** 2) + 0.2D1 * 0.5772156649D&
     0 * q ** 2) / delta) * (0.1D1 - epsilon&
     * exp(-(dble(2 * beta * kk * p) + 0.2D1 * 0.5772156649D0 * ll * q)&
      * t + dble(p * x(1:Nx)) + q * y + eta0)) / (0.1D1 + epsilon * exp(-(dble(2 &
     * beta * kk * p) + 0.2D1 * 0.5772156649D0 * ll * q) * t + dble(p * x(1:Nx)&
     ) + q * y + eta0)) / 2
    ...
    DO j=1,Ny
            !DO i=1,Nx
                    !WRITE(11,*) abs(unax(i,j))**2
                      ...
                      write(11,'(1x,1024e14.4)') (abs(unax(i,j))**2,i=1,Nx)
    ...
    DO n=1,Nt
            ...
            DO j=1,Ny
                    DO i=1,Nx
                            ...
                            potx(i,j)=delta*unax(i,j)*conjg(unax(i,j))
            ...
            DO j=1,Ny
                    DO i=1,Nx
                            vnax(i,j)=exp(0.5d0*dt*(beta*kx(i)*kx(i) + gamma*ky(j)*ky(j))&
                                            *cmplx(0.0d0,1.0d0))*vnax(i,j)
                    END DO
            END DO
            !$OMP END PARALLEL DO
